                ...
                <div class="row align-items-center">
                    <div class="col-md-12">
                        <div class="page-header-title">
                            <h2 class="mb-0">المبالغ المتبقية</h2>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row mb-4">
            <div class="col-12">
                <div class="card border-0 shadow-sm">
                    <div class="card-body">
                        <form method="GET" action="{{ route('property-management.accounting.pending') }}" class="row g-3">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-2 d-flex align-items-end gap-2">
                                ...
                                <a href="{{ route('property-management.accounting.pending') }}" class="btn btn-outline-secondary" title="إعادة تعيين">
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body">
                        <div class="row text-center">
                            <div class="col-md-4">
                                <h6 class="text-muted mb-1">إجمالي المتبقي</h6>
                                <h3 class="mb-0 text-dark fw-bold">{{ number_format($totalPending, 2) }} <small class="text-muted">SAR</small></h3>
                            </div>
                            <div class="col-md-4">
                                <h6 class="text-muted mb-1">عدد الدفعات</h6>
                                <h3 class="mb-0 text-dark fw-bold">{{ $pendingPayments->total() }} <small class="text-muted">دفعة</small></h3>
                            </div>
                            <div class="col-md-4">
                                <h6 class="text-muted mb-1">دفعات متأخرة</h6>
                                <h3 class="mb-0 text-danger fw-bold">{{ $overdueCount }} <small class="text-muted">دفعة</small></h3>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-12">
                <div class="card border-0 shadow-sm">
                    <div class="card-header d-flex justify-content-between align-items-center" style="background-color: #f8f9fa; border-bottom: 1px solid #e9ecef;">
                        <h5 class="mb-0" style="font-weight: 600; color: #212529;">قائمة المبالغ المتبقية</h5>
                        <div class="d-flex gap-2">
                            <a href="{{ route('property-management.accounting.pending.export', ['from_date' => $fromDate, 'to_date' => $toDate, 'search' => $search]) }}" class="btn btn-sm btn-dark">
                    ...
                    <div class="card-body">
                            ...
                            <table class="table table-hover">
                                <thead class="table-light">
                                    <tr>
                                        ...
                                        <th class="border-0 text-center">تاريخ الإصدار</th>
                                        <th class="border-0 text-center">الحالة</th>
                                        <th class="border-0 text-center">أيام التأخير</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            @if($payment->issued_date)
                                                {{ $payment->issued_date->format('Y-m-d') }}
                                            @else
                                                <span class="text-muted">غير محدد</span>
                                            @endif
                                        </td>
                                    ...
                                    <tr>
                                        <td colspan="12" class="text-center py-5">
                                            <div class="text-muted">
                                                <i class="ti ti-clock-off" style="font-size: 48px;"></i>
                                                <p class="mt-3 mb-0">لا توجد مبالغ متبقية في الفترة المحددة</p>
                                            </div>
                                        </td>
                                    </tr>
                                    @endforelse
                                ...
                                <tfoot class="table-light">
                                    <tr>
                                        ...
                                        <th class="text-end">{{ number_format($pendingPayments->sum('rent_value'), 2) }}</th>
                                        <th class="text-end">{{ number_format($pendingPayments->sum('services_value'), 2) }}</th>
                                        <th class="text-end">{{ number_format($pendingPayments->sum('vat_value'), 2) }}</th>
                                        <th class="text-end"><strong>{{ number_format($pendingPayments->sum('total_value'), 2) }}</strong></th>
                                    </tr>
                                </tfoot>
                                @endif
                            </table>
                        ...
                        @if(method_exists($pendingPayments, 'links'))
                            <div class="mt-4">
